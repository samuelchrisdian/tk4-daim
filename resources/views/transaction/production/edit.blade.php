                    ...
                    <div class="breadcrumb-action justify-content-center flex-wrap">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Edit Production</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                            ...
                            <div class="card add-item p-sm-30 p-20 mb-30">
                                <div class="card-body p-0">
                                    <div class="card-header">
                                        <h6 class="fw-500">Edit Production</h6>
                                    </div>
                                    <!-- Start: card body -->
                                    <div class="add-product__body px-sm-40 px-20">
                                        <!-- Start: form -->
                                        <form method="POST" action="{{ route('transaction-production-update', $production->production_id) }}" enctype="multipart/form-data">
                                            @csrf
                                            <div class="form-group mb-20">
                                                ...
                                                <select class="custom-select form-control select-arrow-none ih-medium radius-xs b-light shadow-none color-light fs-14 @error('order_id') is-invalid @enderror" id="order_id" name="order_id" aria-describedby="order_id" required>
                                                    ...
                                                    @php
                                                    if($value->order_id == $production->order_id){
                                                    $check = 'selected';
                                                    }
                                                    @endphp
                                                    <option value="{{ $value->order_id }}" {{ $check }}>{{ $value->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('order_id')
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control @error('qty') is-invalid @enderror" id="qty" name="qty" placeholder="input production qty..." value="{{ $production->qty }}" required>
                                                @error('qty')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control @error('lead_time') is-invalid @enderror" id="lead_time" name="lead_time" placeholder="input production lead time..." value="{{ $production->lead_time }}" required>
                                                @error('lead_time')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
